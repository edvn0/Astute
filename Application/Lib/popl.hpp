...
public:
  ...
  /// @param description the Option's description that will be shown in the help
  /// message
  Option(const std::string& short_name,
...
public:
  ...
  /// @param description the Option's description that will be shown in the help
  /// message
  Value(const std::string& short_name,
        const std::string& long_name,
...
{
public:
  Implicit(const std::string& short_name,
           const std::string& long_name,
           const std::string& description,
...
{
public:
  Switch(const std::string& short_name,
         const std::string& long_name,
         const std::string& description,
...
public:
  ...
  invalid_option(const Option* option,
...
inline std::string
Option::long_name() const
{
  return long_name_;
}

inline std::string
Option::name(OptionName what_name, bool with_hypen) const
{
...
inline std::string
Option::description() const
{
  return description_;
}

inline void
Option::set_attribute(const Attribute& attribute)
{
  attribute_ = attribute;
}

inline Attribute
Option::attribute() const
{
  return attribute_;
}
...
template<class T>
inline void
Value<T>::set_value(const T& value)
{
  clear();
  add_value(value);
}
...
template<class T>
inline T
Value<T>::value_or(const T& default_value, size_t idx) const
{
  if (idx < values_.size())
    return values_[idx];
  else if (default_)
...
template<class T>
inline T
Value<T>::value(size_t idx) const
{
...
template<class T>
inline bool
Value<T>::get_default(std::ostream& out) const
{
  if (!has_default())
    return false;
  out << *this->default_;
...
template<>
inline void
Value<std::string>::parse(OptionName what_name, const char* value)
{
  if (strlen(value) == 0)
...
template<>
inline void
Value<bool>::parse(OptionName /*what_name*/, const char* value)
{
...
template<class T>
inline void
Value<T>::parse(OptionName what_name, const char* value)
{
  T parsed_value;
  std::string strValue;
  if (value != nullptr)
...
template<class T>
inline void
Value<T>::add_value(const T& value)
{
...
template<class T>
inline Argument
Implicit<T>::argument_type() const
{
...
template<class T>
inline void
Implicit<T>::parse(OptionName what_name, const char* value)
{
...
inline void
Switch::parse(OptionName /*what_name*/, const char* /*value*/)
{
  add_value(true);
}

inline Argument
Switch::argument_type() const
{
  return Argument::no;
}
...
inline std::string
OptionParser::description() const
{
  return description_;
}

inline const std::vector<Option_ptr>&
OptionParser::options() const
{
  return options_;
}

inline const std::vector<std::string>&
OptionParser::non_option_args() const
{
  return non_option_args_;
}

inline const std::vector<std::string>&
OptionParser::unknown_options() const
{
...
inline Option_ptr
OptionParser::find_option(char short_name) const
{
  for (const auto& option : options_)
...
template<typename T>
inline std::shared_ptr<T>
OptionParser::get_option(char short_name) const
{
...
inline void
OptionParser::parse(const std::string& ini_filename)
{
  ...
  auto split =
    [trim_copy](const std::string& s) -> std::pair<std::string, std::string> {
    size_t pos = s.find('=');
...
inline std::string
OptionParser::help(const Attribute& max_attribute) const
{
...
inline std::string
ConsoleOptionPrinter::to_string(Option_ptr option) const
{
...
inline std::string
GroffOptionPrinter::to_string(Option_ptr option) const
{
  std::stringstream line;
  if (option->short_name() != 0) {
    line << "-" << option->short_name();
...
operator<<(std::ostream& out, const OptionParser& op)
{
  return out << op.help();
}
